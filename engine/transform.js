/**
 * CWI Attitude Engine v1 — transform.js
 * Zero dependencies. Pure function: (attitude profile) -> Behavior Card.
 *
 * A Behavior Card has three consumable forms:
 *   (a) behavior_config — machine-readable behavior config JSON
 ...
 *
 * Composition law: identity x personality x attitude -> behavior, ALWAYS
 * under bounds. Bounds dominate: precedence is 'bounds-override-attitude'.
 *
 * Works in Node (ESM) and in the browser (classic script -> globalThis.AttitudeEngine).
 */
...
function loadSchema() {
  // Node-only schema autoload; browsers pass opts.schema explicitly.
  if (typeof process !== 'undefined' && process.versions && process.versions.node) {
    const dir = dirname(fileURLToPath(import.meta.url));
    return JSON.parse(readFileSync(join(dir, '..', 'schema', 'attitude-schema.json'), 'utf8'));
  }
  throw new Error('transformAttitude: no schema supplied and no Node fs available — pass opts.schema');
}
...
if (typeof globalThis !== 'undefined') {
  globalThis.AttitudeEngine = { transformAttitude, ENGINE_VERSION, DIM_ORDER };
}
